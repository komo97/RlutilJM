...
void Bar::draw()
{
	...
	for (int i = 0; i < spriteSizeY; i++)
	{
		for (int j = 0; j < spriteSizeX; j++)
		{
			...
			{
				rlUtilJM::AddToBuffer(constraint, constraint, ' ', x + j, y + i, BAR);
			}
			else if (j <= drawPercent && (percent) >= 50)
			{
				rlUtilJM::AddToBuffer(color1, color1, ' ', x + j, y + i, BAR);
			}
			else if (j <= drawPercent && (percent) < 50 && (percent) > 25)
			{
				rlUtilJM::AddToBuffer(color2, color2, ' ', x + j, y + i, BAR);
			}
			else if (j <= drawPercent && (percent) <= 25)
			{
				rlUtilJM::AddToBuffer(color3, color3, ' ', x + j, y + i, BAR);
			}
			else
			{
