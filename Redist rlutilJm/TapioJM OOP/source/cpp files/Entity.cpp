	...
	if (type == CHARACTER || type == ENEMY)
	{
		...
		for (i = 0; i < spriteSizeY; i++)
		{
			for (j = 0; j <  spriteSizeX; j++)
			{
				...
				{
					rlUtilJM::AddToBuffer( color1,  bg1,  letter1,  x + j,  y + i,  type);
				}
				else if (( sprite[i][j]) & CHARACTER2)
				{
					rlUtilJM::AddToBuffer( color2,  bg2,  letter2,  x + j,  y + i,  type);
				}
				else if (( sprite[i][j]) & CHARACTER3)
				{
					rlUtilJM::AddToBuffer( color3,  bg3,  letter3,  x + j,  y + i,  type);
				}
				else if (( sprite[i][j]) & CHARACTER4)
				{
					rlUtilJM::AddToBuffer( color4,  bg4,  letter4,  x + j,  y + i,  type);
				}
				else if (( sprite[i][j]) & WEAPON)
				{
					rlUtilJM::AddToBuffer( weapon, BLACK,  charWeapon,  x + j,  y + i,  type);
				}
				else if (( sprite[i][j]) & CLEAR)
				{
					rlUtilJM::AddToBuffer(BLACK, BLACK, ' ',  x + j,  y + i, CLEAR);
				}
				else
				{
